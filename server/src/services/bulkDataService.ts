...
export function getLastImportTime(): string | null {
    try {
        ...
        const result = db.prepare('SELECT value FROM metadata WHERE key = ?').get('last_import') as { value: string } | undefined;
...
function setLastImportTime(timestamp: string): void {
    const db = getDatabase();
    db.prepare('INSERT OR REPLACE INTO metadata (key, value) VALUES (?, ?)').run('last_import', timestamp);
}
...
export function shouldImport(): boolean {
    ...
    const now = new Date();
    const daysSinceImport = (now.getTime() - lastImportDate.getTime()) / (1000 * 60 * 60 * 24);
...
export async function downloadAndImportBulkData(): Promise<{
    ...
    console.log('[Bulk Import] Starting bulk data import...');

    // Get bulk data info
    const bulkInfo = await getBulkDataInfo();
    console.log(`[Bulk Import] Downloading from: ${bulkInfo.download_uri}`);
    console.log(`[Bulk Import] File size: ${(bulkInfo.size / 1024 / 1024).toFixed(1)} MB`);
    ...
    arrayStreamer.on('data', ({ value }: { value: ScryfallBulkCard }) => {
        ...
        if (batch.length >= BATCH_SIZE) {
            ...
            if (cardsProcessed % 10000 === 0) {
                console.log(`[Bulk Import] Processed ${cardsProcessed} cards... (${totalInserted} new, ${totalUpdated} updated)`);
            }
        }
    });
    ...
    console.log(`[Bulk Import] Complete! Processed ${cardsProcessed} cards in ${(durationMs / 1000).toFixed(1)}s`);
    console.log(`[Bulk Import]   └─ ${totalInserted} new, ${totalUpdated} updated`);
    console.log(`[Bulk Import] Total cards in database: ${getCardCount()}`);

    return { cardsImported: cardsProcessed, cardsNew: totalInserted, cardsUpdated: totalUpdated, durationMs };
}
 ...
 */
function convertBulkCard(bulk: ScryfallBulkCard): ScryfallApiCard & { id: string; oracle_id?: string } {
    return {
        id: bulk.id,
