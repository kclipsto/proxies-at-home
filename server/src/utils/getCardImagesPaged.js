...
exports.getImagesForCardInfo = getImagesForCardInfo;
exports.getCardsWithImagesForCardInfo = getCardsWithImagesForCardInfo;
exports.getCardDataForCardInfo = getCardDataForCardInfo;
exports.getScryfallPngImagesForCard = getScryfallPngImagesForCard;
exports.getScryfallPngImagesForCardPrints = getScryfallPngImagesForCardPrints;
 ...
 */
async function fetchAllPages(query, extractor) {
 ...
 */
async function searchScryfallWithFallback(searchFn, queryBuilder, language, fallbackToEnglish) {
 ...
 */
async function getImagesForCardInfo(cardInfo, unique = "art", language = "en", fallbackToEnglish = true) {
    const { name, set, number } = cardInfo || {};
    // Helper to build query based on strategy
    const executeStrategy = (queryTemplate) => {
    ...
    if (unique === "prints" && set && number) {
        const results = await executeStrategy((lang) => `set:${set} number:${escapeColon(number)} name:"${name}" include:extras unique:prints lang:${lang}`);
        if (results.length)
            return results;
    }
    // 2) Set + name (all printings in set for that name)
    if (unique === "prints" && set && !number) {
        const results = await executeStrategy((lang) => `set:${set} name:"${name}" include:extras unique:prints lang:${lang}`);
        if (results.length)
            return results;
    }
    // 3) Name-only search - this is the main strategy for unique:art
    return executeStrategy((lang) => `!"${name}" include:extras unique:${unique} lang:${lang}`);
}
/**
 * Returns full card data (including images and metadata) for a CardInfo.
 */
async function getCardsWithImagesForCardInfo(cardInfo, unique = "art", language = "en", fallbackToEnglish = true) {
    const { name, set, number } = cardInfo || {};
    const executeStrategy = (queryTemplate) => {
    ...
    // 3) Name-only
    const results = await executeStrategy((lang) => `!"${name}" include:extras unique:${unique} lang:${lang}`);
 ...
 */
async function getCardDataForCardInfo(cardInfo, language = "en", fallbackToEnglish = true) {
    const { name, set, number } = cardInfo || {};
    if (!name)
        return null;
    const executeStrategy = (queryTemplate) => {
    ...
    if (set && number) {
        const cards = await executeStrategy((lang) => `set:${set} number:${escapeColon(number)} name:"${name}" include:extras lang:${lang}`);
        if (cards.length)
            return cards[0];
    }
    // Strategy 2: Set + name
    if (set) {
        const cards = await executeStrategy((lang) => `set:${set} name:"${name}" include:extras unique:prints lang:${lang}`);
        if (cards.length)
            return cards[0];
    }
    // Strategy 3: Name-only exact match
    // We use unique:art to get different art options, and order:released to prefer newer cards
    const cards = await executeStrategy((lang) => `!"${name}" include:extras unique:art order:released lang:${lang}`);
    return cards[0] || null;
}
async function getScryfallPngImagesForCard(cardName, unique = "art", language = "en", fallbackToEnglish = true) {
    return searchScryfallWithFallback(fetchPngsByQuery, (lang) => `!"${cardName}" include:extras unique:${unique} lang:${lang}`, language, fallbackToEnglish);
}
async function getScryfallPngImagesForCardPrints(name, language = "en", fallbackToEnglish = true) {
    return searchScryfallWithFallback(fetchPngsByQuery, (lang) => `!"${name}" include:extras unique:prints lang:${lang}`, language, fallbackToEnglish);
}
