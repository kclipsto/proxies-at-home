...
streamRouter.post("/cards", async (req, res) => {
    ...
    try {
        ...
        const cardQueries = (0, cardUtils_js_1.normalizeCardInfos)(Array.isArray(req.body.cardQueries) ? req.body.cardQueries : null, null, language);
        ...
        for (const ci of cardQueries) {
            ...
            try {
                ...
                const scryfallCards = await (0, getCardImagesPaged_js_1.getCardsWithImagesForCardInfo)(ci, "art", language);
                if (scryfallCards && scryfallCards.length > 0) {
                    ...
                    if (imageUrls.length > 0) {
                        ...
                        if ((!colors || !mana_cost) && primaryCard.card_faces && primaryCard.card_faces.length > 0) {
                            if (!colors)
                                colors = primaryCard.card_faces[0].colors;
                            if (!mana_cost)
                                mana_cost = primaryCard.card_faces[0].mana_cost;
                        }
                        const cardToSend = {
                            name: ci.name,
                            set: primaryCard.set,
