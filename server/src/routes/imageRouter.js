...
const getCardImagesPaged_js_1 = require("../utils/getCardImagesPaged.js");
const cardUtils_js_1 = require("../utils/cardUtils.js");
...
function pLimit(concurrency) {
    ...
    return (fn) => new Promise((resolve, reject) => {
        ...
        if (active < concurrency)
            run(fn, wrappedResolve, wrappedReject);
        else
            q.push([fn, wrappedResolve, wrappedReject]);
    });
}
...
const cacheDir = path_1.default.join(__dirname, "..", "cached-images");
...
const uploadDir = path_1.default.join(__dirname, "..", "uploaded-images");
...
function cachePathFromUrl(originalUrl) {
    const hash = crypto_1.default.createHash("sha1").update(originalUrl).digest("hex");
    ...
    return path_1.default.join(cacheDir, `${hash}${ext}`);
}
imageRouter.post("/", async (req, res) => {
    ...
    const fallbackToEnglish = typeof req.body.fallbackToEnglish === "boolean" ? req.body.fallbackToEnglish : true;
    if (!cardQueries && !cardNames) {
        return res.status(400).json({ error: "Provide cardQueries (preferred) or cardNames." });
    }
    const infos = (0, cardUtils_js_1.normalizeCardInfos)(cardQueries, cardNames, language);
    const started = Date.now();
    try {
        const results = await Promise.all(infos.map((ci) => limit(async () => {
            ...
            const task = (async () => {
                const imageUrls = await (0, getCardImagesPaged_js_1.getImagesForCardInfo)(ci, unique, ci.language, fallbackToEnglish);
                return {
                    name: ci.name,
                    set: ci.set,
                    number: ci.number,
...
imageRouter.post("/images-stream", async (req, res) => {
    ...
    const unique = req.body.cardArt || "art";
    const language = (req.body.language || "en").toLowerCase();
    const fallbackToEnglish = typeof req.body.fallbackToEnglish === "boolean"
        ? req.body.fallbackToEnglish
        : true;
    const infos = (0, cardUtils_js_1.normalizeCardInfos)(cardQueries, null, language);
...
imageRouter.get("/proxy", async (req, res) => {
    ...
    try {
        if (fs_1.default.existsSync(localPath)) {
            ...
            try {
                const now = new Date();
                fs_1.default.utimesSync(localPath, now, now);
            }
            catch {
...
// -------------------- maintenance & uploads --------------------
imageRouter.delete("/", (_req, res) => {
    const started = Date.now();
    fs_1.default.readdir(cacheDir, (err, files) => {
        if (err) {
