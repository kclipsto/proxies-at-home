...
export function DecklistUploader({
  ...
  const extractTokenPrintFromUri = (
    ...
    try {
      ...
      const tokenIdx = parts.findIndex((p) =>
        ["token", "tokens", "tok", "tk"].includes(p)
      );
  ...
  return (
    <div
      className={`space-y-4 ${mobile ? "landscape:flex landscape:flex-col landscape:h-full landscape:space-y-0 landscape:gap-4" : ""}`}
    >
      <div
        className={`space-y-1 ${mobile ? "landscape:flex-1 landscape:flex landscape:flex-col" : ""}`}
      >
      ...
      <div className={`space-y-1 ${mobile ? "landscape:hidden" : ""}`}>
        <div className="flex items-center justify-between">
          ...
          <AutoTooltip
            content="Used for Scryfall lookups"
            mobile={mobile}
