...
describe("scryfallApi", () => {
  ...
  describe("getImages", () => {
    it("should extract large image_uris if available", () => {
      const card = {
        image_uris: { large: "http://large.jpg", normal: "http://normal.jpg" },
      };
    ...
    it("should extract images from card_faces for DFCs", () => {
      ...
      expect(getImages(card as never)).toEqual([
        "http://front.jpg",
        "http://back.jpg",
      ]);
    });
  ...
  describe("constructScryfallQuery", () => {
    it("should append unique:prints by default", () => {
      expect(constructScryfallQuery("Sol Ring")).toBe("Sol Ring unique:prints");
    });

    it("should append unique:prints if mode is prints", () => {
      expect(constructScryfallQuery("Sol Ring", "prints")).toBe(
        "Sol Ring unique:prints"
      );
    });

    it("should append unique:cards if mode is cards", () => {
      expect(constructScryfallQuery("Sol Ring", "cards")).toBe(
        "Sol Ring unique:cards"
      );
    });

    it("should append unique if query has Scryfall syntax but no unique param", () => {
      // This was the bug: (set:eoe) triggers "has syntax" check and skipped appending unique
      expect(constructScryfallQuery("Forest (set:eoe)", "prints")).toBe(
        "Forest (set:eoe) unique:prints"
      );
      expect(constructScryfallQuery("c:r t:creature", "cards")).toBe(
        "c:r t:creature unique:cards"
      );
    });

    it("should not append unique if query already has unique", () => {
      expect(constructScryfallQuery("Sol Ring unique:art")).toBe(
        "Sol Ring unique:art"
      );
      expect(constructScryfallQuery("unique:prints Forest")).toBe(
        "unique:prints Forest"
      );
    });

    it("should handle empty query", () => {
      expect(constructScryfallQuery("")).toBe("");
      expect(constructScryfallQuery("   ")).toBe("");
    });
  });
  ...
  describe("searchCards", () => {
    ...
    it("should return empty array on failure", async () => {
      ...
      await expect(searchCards("Fail")).rejects.toThrow(
    ...
    it("should throw friendly error for 404", async () => {
      ...
      await expect(searchCards("Unknown")).rejects.toThrow(
    ...
    it("should throw friendly error for 500", async () => {
      ...
      await expect(searchCards("ServerError")).rejects.toThrow(
    ...
    it("should throw network error for request timeout", async () => {
      ...
      await expect(searchCards("NetworkError")).rejects.toThrow(
    ...
    it("should rethrow cancel errors", async () => {
      ...
      (axios.isCancel as unknown as ReturnType<typeof vi.fn>).mockReturnValue(
        true
      );
  ...
  describe("fetchCardWithPrints", () => {
    ...
    it("should return card with prints if search and print fetch succeed", async () => {
      ...
      expect(result?.prints?.[0]).toMatchObject({
        imageUrl: "http://example.com/print1.jpg",
        set: "cmd",
        number: "1",
      });
      expect(result?.prints?.[1]).toMatchObject({
        imageUrl: "http://example.com/print2.jpg",
        set: "2xm",
        number: "274",
      });
